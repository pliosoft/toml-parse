...
pfdecimal = do
  int <- many1 (digit `sepBy1` (optional (char '_')))
...
convertDT ::
  (ZonedTime -> Token) ->
  ( (Integer, Int, Int),
    ((Int, Int, Int, Maybe Rational), Maybe (Char, Int, Int))
  ) ->
  Maybe Token
convertDT f ((y, m, d), ((h, mi, sec, r), os)) =
  do
    fd <- fromGregorianValid y m d
...
dMultilineString pos = do
  (raw, v) <- match mlparse
  return (QuotedStringT pos v, second (+ (T.length raw)) pos)
  where
    mlparse = do
      void $ dMultilineToken
      skipWhile (\c -> c == '\n')
      skipMany multilineWhitespace
...
manyWithPos ::
  ...
  Position ->
  Parser ([a], Position)
manyWithPos p pos = do
  r <- fmap (first Just) (p pos) <|> pure (Nothing, pos)
  case r of
    (Nothing, _) ->
      return ([], pos)
    (Just x, newpos@(!_, !_)) ->
      do
        (xs, finalpos) <- manyWithPos p newpos
