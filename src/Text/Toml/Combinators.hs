...
lbrace = satisfy it
  where
    it (LeftBraceT _) = Just ()
    it _ = Nothing

bool :: Parser Bool
bool = satisfy it
  where
    it (KeywordTrueT _) = Just True
...
integer = satisfy it
  where
    it (IntegerT _ i _) = Just i
    it _ = Nothing

float :: Parser Double
float = satisfy it
  where
    it (FloatT _ f _) = Just f
    it _ = Nothing
